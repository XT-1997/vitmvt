...
class AlphaLayer(nn.Module):
    """Alpha Layer form DMCP algorithm.

    Args:
        channels (int): The input (and output) channels of the Alpha layer.
        min_width (int): The minimum search range of the alpha layer.
        max_width (int): The maximum search range of the alpha layer.
        offset (int): Search step size of the alpha layer.
        prob_type(float): Calculation method. Default:exp
    """

    def __init__(self, channels, width, prob_type='exp'):
        ...
        self.channels = \
            channels if isinstance(channels, int) else channels.data[-1]

        ch_indice = self._get_ch_indice(min_width, max_width, offset,
                                        self.channels)
        self.min_ch = ch_indice[0]
        for i in range(1, len(ch_indice) - 1):
            assert ch_indice[i + 1] - ch_indice[i] \
                == ch_indice[i] - ch_indice[i - 1]
        # no channels to pruned if num_groups == 0
        self.num_groups = len(ch_indice) - 1
        self.group_size = ch_indice[1] - ch_indice[0] \
            if self.num_groups > 0 else 0
        assert self.group_size * self.num_groups + self.min_ch == self.channels
    ...
    def expected_sampling(self):
        """Expected sampling (ES): set the number of channels to be expected
        channels."""
        expected = round(self.expected_channel().item() - 1e-4)
        candidate = [
            self.min_ch + self.group_size * i
    ...
    def extra_repr(self) -> str:
        return 'channels={}, min_ch={}, num_groups={}'.format(
            self.channels, self.min_ch, self.num_groups)

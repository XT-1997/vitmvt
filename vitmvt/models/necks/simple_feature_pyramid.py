...
from ..mutables import ConvTranspose2dSlice, LayerNormSlice, SyncBatchNormSlice
...
#         self.ln = NORM_LAYERS.build(dict(type='LayerNormSlice', embedding_dim=embed_dim))
#         # self.ln = LayerNormSlice(embed_dim, eps=1e-6)
...
class Norm2d(nn.Module):
    ...
    def __init__(self, embed_dim):
        super().__init__()
        self.syncbn = SyncBatchNormSlice(embed_dim)
    
    def forward(self, x):
        return self.syncbn(x)
...
class SFP(BaseModule):
    ...
    def forward(self, inputs):
        ...
        if len(inputs) == 1:
            ...
            features = [
                top_down(inputs[_]) for _, top_down in enumerate(self.top_downs)
            ]
...
if __name__ == '__main__':
    ...
    model = SFP(in_channels, 256, len(in_channels))
    import torch
    xx = []
    x = torch.randn(16, 576, 14, 14)
    xx.append(x)
    for i in model(tuple(xx)):
        print(i.shape)